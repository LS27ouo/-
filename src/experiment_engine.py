...
def run_experiment(exp, run_args,
    ...
    with tf.compat.v1.Graph().as_default():
        tbw = tf.compat.v1.summary.FileWriter(logs_dir)
    ...
    if tensorBoardWriter is not None:
        for i in range(len(loss_names)):
            tensorBoardWriter.add_summary(
                tf.compat.v1.Summary(value=[tf.compat.v1.Summary.Value(tag=loss_names[i], simple_value=loss_vals[i]), ]), iter_count)
            if i >= len(loss_vals):
                break
